...
/** Optional seed for a brand-new recipe (e.g. created inline from a POS line). */
export type RecipeEditorPrefill = {
  name?: string;
...
function createDefaultForm(
  ...
  return {
    name: prefill?.name ?? "",
    category: prefill?.category ?? "mains",
    serves: 1,
    wastagePercent: 0,
    gpTargetPercent: 65,
    instructions: "",
...
function createPrefillIngredient(name: string): RecipeIngredient {
  return {
...
function createDefaultIngredients(
  ...
  if (!recipe) {
    return (prefill?.ingredientNames ?? [])
      .map((name) => name.trim())
      .filter(Boolean)
      .map(createPrefillIngredient);
  }
...
export function RecipeEditorDrawerContent({
  ...
  const [form, setForm] = useState<RecipeFormState>(() => createDefaultForm(recipe, prefill));
  const [ingredients, setIngredients] = useState<RecipeIngredient[]>(() =>
    createDefaultIngredients(recipe, prefill),
  );
  ...
  useEffect(() => {
    setForm(createDefaultForm(recipe, prefill));
    setIngredients(createDefaultIngredients(recipe, prefill));
  }, [recipe, prefill]);
          ...
          <div className="min-h-0 flex-1 overflow-y-auto pr-1">
            <TabsContent value="details" className="mt-3">
              <div className="space-y-4 rounded-lg border p-4">
                ...
                {isNewRecipe && prefill?.suggestedPriceCents ? (
                  <p className="text-muted-foreground text-xs">
                    POS sell price: {formatCurrency(prefill.suggestedPriceCents)} — set your GP target below to
                    compare against the suggested price.
                  </p>
                ) : null}
