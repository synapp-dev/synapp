...
import { RawItemsTable } from "@/entities/supplier-raw-items/components/raw-items-table";
...
export function SupplierDetailPageClient({
  ...
  const [activeTab, setActiveTab] = useState(inventorySetupMode ? "raw-items" : "details");
    ...
    <>
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        <TabsList className="flex h-auto flex-wrap gap-1">
          ...
          {inventorySetupMode ? (
            <TabsTrigger value="raw-items" className="gap-1.5">
              <Package className="h-4 w-4" />
              Raw items ({rawItemTotal})
            </TabsTrigger>
          ) : (
            ...
            <div className="grid gap-4 md:grid-cols-3">
              ...
              <div className="space-y-2 md:col-span-2">
                ...
                <Input
                  ...
                  onChange={(e) =>
                    setDraft((d) => (d ? { ...d, orderingEmail: e.target.value } : d))
                  }
                />
        ...
        {inventorySetupMode ? (
          <TabsContent value="raw-items" className="mt-4 space-y-4">
            <RawItemsTable organisation={organisation} venue={venue} supplierId={supplierId} />
          </TabsContent>
        ) : (
        ...
        <TabsContent value="prices" className="mt-4 space-y-4">
          <Card className="space-y-4 p-6">
            <h3 className="text-sm font-semibold tracking-wide text-muted-foreground uppercase">Notes</h3>
            <Textarea
              rows={6}
              value={draft.notes}
              onChange={(e) => setDraft((d) => (d ? { ...d, notes: e.target.value } : d))}
