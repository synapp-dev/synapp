...
vi.mock("@/entities/stock-counts/components/stock-count-flow-page", () => ({
  StockCountFlowPage: () => null,
}));
...
import StockCountFlowRoute from "./page";
...
/**
 * Rebuilds the query key exactly as StockCountFlowPage does on first render:
 * useStockCountDetailQuery({ organisation, venue, countId }).
 */
...
async function renderPageDehydratedState(): Promise<DehydratedQueries> {
  const element = await StockCountFlowRoute(pageParams());
  return (element as unknown as { props: { state: DehydratedQueries } }).props
    .state;
}

describe("StockCountFlowRoute RSC prefetch", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  });
  ...
  it("prefetches under the exact query key the client computes on first render", async () => {
    ...
    const data = {
      id: "count-1",
      status: "in_progress",
      entries: [{ ingredientId: "ing-1", countedQty: null }],
    };
