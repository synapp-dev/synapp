...
function hourlyRateCentsForStaff(id: string): number {
  return 2600 + (hashString(id) % 15) * 100;
}
...
export function RosterPageClient({ organisation, venue }: RosterPageClientProps) {
  ...
  const staffShiftMap = useMemo(() => {
    ...
    for (const shift of shifts) {
      ...
      const inner = map.get(shift.staffId)!;
  ...
  const costSummary = useMemo(() => {
    ...
    for (const shift of shifts) {
      ...
      const rate = hourlyRateCentsForStaff(shift.staffId);
    ...
    const budgetCents = 2_000_00;

    return { totalHours, totalCostCents, dailyCosts, budgetCents };
  }, [shifts]);
  ...
  const headerMetrics = useMemo(() => {
    ...
    for (const shift of subset) {
      ...
      const rate = hourlyRateCentsForStaff(shift.staffId);
      totalHours += hours;
      totalCostCents += Math.round(hours * rate);
  ...
  const staffDayHours = useMemo(() => {
    const map = new Map<string, number>();
    for (const shift of shifts) {
      if (shift.dayIndex !== safeDayIdx) continue;
      const prev = map.get(shift.staffId) ?? 0;
      map.set(shift.staffId, prev + shiftHours(shift));
    }
    return map;
    ...
    <section className="flex min-h-0 flex-1 flex-col gap-3 overflow-hidden">
      ...
      <div className="flex shrink-0 flex-wrap items-center justify-between gap-3">
        ...
        <div className="flex flex-wrap items-center justify-end gap-x-2 gap-y-1.5">
          ...
          <span
            ...
            aria-hidden
          />
          <Button size="sm" className="gap-1.5" onClick={() => toast.info("Publish coming soon")}>
      ...
      <div className="flex w-full min-w-0 shrink-0 flex-wrap items-center gap-3 gap-y-2">
        <div className="flex min-w-0 flex-wrap items-center gap-2">
          ...
          >
            <LayoutTemplate className="h-3.5 w-3.5 text-sky-600" />
            Template
          </Button>
          <Button
            variant="ghost"
            size="sm"
            className="h-8 gap-1 text-xs"
            onClick={() => toast.info("Compliance check")}
          >
            <AlertTriangle className="h-3.5 w-3.5 text-orange-500" />
            Compliance
          </Button>
          <Button variant="ghost" size="sm" className="h-8 gap-1 text-xs" onClick={() => toast.info("Auto-fill")}>
            <Wand2 className="h-3.5 w-3.5 text-purple-500" />
            Auto-fill
          </Button>
          <Button variant="ghost" size="sm" className="h-8 gap-1 text-xs" onClick={() => toast.info("Quick Build")}>
                            ...
                            >
                              {shift ? (
                                <button
                                  type="button"
                                  className={cn(
                                    "flex w-[min(100%,7.5rem)] flex-col rounded-md px-2 py-1 text-left transition-shadow hover:shadow-md",
      ...
      >
        ...
        >
          ...
          <SheetFooter className="flex-row justify-end gap-2 sm:justify-end">
            <Button type="button" variant="outline" onClick={closeEditShift} disabled={editSaving}>
              Cancel
            </Button>
            <Button
              type="button"
