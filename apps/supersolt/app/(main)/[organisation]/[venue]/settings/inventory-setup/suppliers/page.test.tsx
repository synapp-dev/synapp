...
import InventorySetupSuppliersPage from "./page";
 ...
 * first render in inventorySetupMode: store defaults mapped through the same
 * "all" -> undefined / trim-or-undefined / yes-no-boolean rules as its
 * useSuppliersQuery call, with inventorySource pinned to true (setup mode
 * only shows suppliers kept as inventory sources). If the store defaults or
 * that mapping ever drift from the page's prefetch constant, the hash
 * comparison below fails and the silent-spinner regression is caught here
...
async function renderPageDehydratedState(): Promise<DehydratedQueries> {
  const element = await InventorySetupSuppliersPage(pageParams());
...
describe("InventorySetupSuppliersPage RSC prefetch", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("prefetches both queries under the exact keys the client computes on first render", async () => {
    ...
    const xeroHash = hashKey(xeroKeys.venueConnection("org-1", "ven-1"));
  ...
  it("renders with no prefetched state when a service throws", async () => {
    ...
    vi.mocked(getVenueXeroConnectionSummary).mockRejectedValue(
