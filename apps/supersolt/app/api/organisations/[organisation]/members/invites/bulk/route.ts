...
export async function POST(
  ...
  const emails = Array.isArray(body.emails)
    ? body.emails.filter((e): e is string => typeof e === "string" && e.trim().length > 0)
    : [];
  ...
  if (emails.length === 0 || venueIds.length === 0) {
    return NextResponse.json(
      {
        data: null,
        error: {
          message: "emails and venueIds are required",
          status: 400,
  ...
  try {
    const data = await membersInviteService.createBulkInvites(ctx, admin, {
