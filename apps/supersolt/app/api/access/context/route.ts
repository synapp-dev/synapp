...
type AccessContextOrganisation = {
  id: string;
  name: string;
  slug: string;
  logoUrl: string | null;
...
export async function GET() {
  ...
  if (typedMemberships.length === 0) {
    return NextResponse.json({ data: { organisations: [] }, error: null });
  }
    ...
    return NextResponse.json(
      {
        data: null,
        error: {
          message: userVenueError.message,
          status: 500,
        },
      },
      { status: 500 }
  ...
  const roleById = new Map(
    (roleRows ?? []).map((r) => [r.id, r as RoleRow])
  );
  ...
  const venuesByMembershipId = new Map<string, AccessContextVenue[]>();
  ...
  const organisations: AccessContextOrganisation[] = typedMemberships
    .map((membership) => {
      const orgRole = roleById.get(membership.role_id);
      if (!orgRole) {
        return null;
    ...
    .filter((o): o is AccessContextOrganisation => o !== null && o.slug.length > 0)
