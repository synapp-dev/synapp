-- Generalize the profile "anthem" to support multiple providers. Renames
-- spotify_track_url -> anthem_url and widens the shape guard to allow either a
-- canonical Spotify track URL or a canonical SoundCloud track URL. Provider is
-- derived from the URL prefix at render time (no separate column needed).
