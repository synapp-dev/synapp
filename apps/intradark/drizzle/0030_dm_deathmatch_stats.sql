-- Deathmatch stats: raw per-event firehose + derived leaderboard view.
-- Fully separate from MatchZy/PUG (own tables, route, secret, plugin). See
-- docs/cs2-stats-leaderboard.md. Keyed by steamid64 (text, matches public.players
-- / public.steam_profiles). Public read; writes via service role only. The events
-- table is NOT FK'd to players: the DM server reports every connected steamid64
-- (incl. untracked pubbers), and a hard FK would reject those inserts and drop data.
-- Idempotent: safe if tables/policies/view already exist (partial apply via MCP).
...
CREATE TABLE IF NOT EXISTS public.dm_kill_events (
    ...
    map_name           TEXT,
    -- 'death' | 'hurt' | 'connect' | 'disconnect' | ... (kept open; raw has the rest)
...
-- Dedupe key: ON CONFLICT (event_id) DO NOTHING on ingest.
...
-- Derived live from dm_kill_events (always correct; promote to a materialized view
-- or rollup table only if it gets slow at scale). LEFT JOINs profile tables so
-- untracked pub players still appear (just without name/avatar/flag).
-- security_invoker=true: the view respects the querying user's RLS rather than the
-- owner's (avoids the security_definer_view lint). Safe — dm_kill_events,
-- steam_profiles and players all have public-read (USING true) SELECT policies.
CREATE OR REPLACE VIEW public.dm_player_stats
WITH (security_invoker = true) AS
WITH contributions AS (
  -- One row per (player, scoring contribution). A kill requires attacker present
  -- and attacker <> victim (excludes suicides / world deaths).
