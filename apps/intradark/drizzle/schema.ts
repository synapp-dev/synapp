import { pgTable, pgSchema, index, foreignKey, check, uuid, text, timestamp, jsonb, varchar, bigserial, boolean, inet, bigint, uniqueIndex, smallint, json, pgPolicy, integer, unique } from "drizzle-orm/pg-core"
...
export const steamProfiles = pgTable("steam_profiles", {
	...
	pgPolicy("Allow update steam_profiles", { as: "permissive", for: "update", to: ["public"], using: sql`true` }),
	pgPolicy("Allow insert steam_profiles", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("Allow public read steam_profiles", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const userProfiles = pgTable("user_profiles", {
	...
	pgPolicy("Users can delete their own profile", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
