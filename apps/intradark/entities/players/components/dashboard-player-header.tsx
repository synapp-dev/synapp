...
// NOTE: standalone copy of <PlayerHeader> for the dashboard hero. Intentionally
// duplicated (not a shared component) so the dashboard card can diverge freely.
// First divergence: the Veritas/legitimacy column is dropped — that summary only
// belongs on the full player profile page.
...
function SteamIcon({ className }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 88.32 88.47" fill="currentColor" aria-hidden>
...
export interface DashboardPlayerHeaderProps {
  ...
  teamLogoSrc?: string;
  /** Gates the staggered entrance; while false a loading placeholder shows. */
  start?: boolean;
...
export function DashboardPlayerHeader({
  ...
  const watermarkLogoSrc = teamLogoSrc ?? team?.avatar ?? DEFAULT_THEME.logoSrc;
        ...
        >
          ...
          <div className="relative z-[2] flex h-full min-h-0 w-full min-w-0 flex-row items-stretch overflow-hidden">
            <div className="relative flex h-full min-h-0 shrink-0 overflow-visible pl-3 sm:pl-4 md:pl-5">
              {/* Intradark star / team watermark — slides up with the avatar. */}
                  ...
                  {profileSocialLinks.map((link, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <SocialIcon kind={link.icon} className={SOCIAL_ICON_CLASS} />
                      </Link>
                    </StaggeredAnimation>
                  ))}
                </nav>
            ...
            <div className="relative flex h-full min-h-0 min-w-0 flex-1 flex-col gap-5 overflow-hidden p-6 pb-16 md:pb-6">
              <div className="flex min-h-0 w-full min-w-0 flex-1 flex-col justify-end gap-2 overflow-hidden">
                <div className={alias.clipClassName}>
                  <div className={alias.innerClassName} style={alias.innerStyle}>
                    <h1 className="text-4xl font-extrabold tracking-tight text-white sm:text-5xl lg:text-6xl">
                      {name}
                    </h1>
                    {isMember ? <GoldVerifiedBadge /> : null}
                    {!subtitle && countryFlag ? (
                      <CountryFlag code={countryFlag} className="mt-2 sm:mt-3" />
                    ) : null}
                  </div>
                </div>
                {subtitle ? (
                ...
                <div className={roleTeam.clipClassName}>
                  <div className={roleTeam.innerClassName} style={roleTeam.innerStyle}>
                ...
                <div className={stats.clipClassName}>
                  <div className={stats.innerClassName} style={stats.innerStyle}>
                    <Separator className="my-0.5 w-1/3 max-w-1/12 bg-white/10" />
