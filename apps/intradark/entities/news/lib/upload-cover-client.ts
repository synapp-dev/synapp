...
export type UploadNewsCoverResult =
  | { ok: true; objectPath: string; publicUrl: string }
  | { ok: false; message: string };

/** Client-side: sign + PUT a news cover image to `intradark-media`, returning its public URL. */
export async function uploadNewsCoverFile(
  articleId: string,
  file: File,
): Promise<UploadNewsCoverResult> {
  ...
  const signRes = await fetch("/api/news/cover-upload-url", {
    method: "POST",
  ...
  if (!signRes.ok || !signJson.signedUrl || !signJson.publicUrl) {
    return {
      ok: false,
      message:
        signJson.error ?? `Could not start upload (HTTP ${signRes.status}).`,
    };
  }
