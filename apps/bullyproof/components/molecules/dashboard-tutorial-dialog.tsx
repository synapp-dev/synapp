...
type DashboardTutorialDialogProps = {
  open: boolean;
  onOpenChange: (open: boolean) => void;
};

export function DashboardTutorialDialog({
  open,
  onOpenChange,
}: DashboardTutorialDialogProps) {
  ...
  const handleGotIt = async () => {
    if (dontShowAgain) {
      try {
        await dismissDialog.mutateAsync("dashboard");
      } catch (error) {
        ...
        <DialogHeader className="text-center shrink-0">
          ...
          <div className="flex items-center gap-4">
            <DialogDescription className="flex-1 text-left m-0">
              This is your dashboard where you can access all your classes, lessons, and resources. Use the sidebar to navigate between different sections of the platform. You can switch between schools using the school switcher at the top of the sidebar.
            </DialogDescription>
