...
| 39 - Address / email domain save | Reported not saving | `7c7bf90` | CORRECTED in the 7 July full re-test: the defect was real. The write chain was intact (save persisted), but the client School transform dropped address/emailDomain, so the admin drawer displayed empty fields after reload, which reads as a failed save (exactly Glenn's report). Fixed by carrying the fields through the transform; verified live on the exact edit-save-reload flow. The earlier "verified fixed" note was wrong and is superseded by this row |
...
## Full re-test (7 July 2026, afternoon)

Every Variation/SOW deliverable and every fix re-tested in the browser in one
pass; results per deliverable in `variation-deliverables-verification.md`.
Highlights: full lesson lifecycle driven end to end on the test school
(create on behalf, access-policy block for non-owner, cancel via the update
path, appearance in history and exports); admin export CSV contents inspected
on disk (all pack sections present with live data); item 39 reproduced,
root-caused and fixed (`7c7bf90`); all wizard defect fixes re-verified; 97
unit tests and typecheck green.
