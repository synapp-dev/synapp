# TAX INVOICE

| | |
|---|---|
| Supplier: | Intradark Pty Ltd |
| ABN: | 38 696 182 457 |
| Invoice number: | [INV-2026-___] |
| Invoice date: | 7 July 2026 |
| Bill to: | Amayda Pty Ltd, Attn: Glenn Rushton, Director [insert ABN / address] |

## Items

| Description | Amount |
|---|---|
| Bullyproof platform: completion and handover payment under the Formal Variation Agreement, clause 4(b). Completion of all deliverables under the Statement of Work (Schedule 1), full handover including source code and repository control, and acceptance under clause 7. Per the Completion and Deliverables Register, FINAL issue dated 7 July 2026. | $100,000.00 |
| GST (10%) | $10,000.00 |
| **Total payable (GST inclusive)** | **$110,000.00** |

## Payment terms

Payable on written Acceptance under clause 7 and completion of handover, per clause 4(b) of the Formal Variation Agreement. Deemed Acceptance under clause 7.4 applies if no response is received within the UAT Period.

Note: this invoice covers the clause 4(b) fee balance only. The salary component remainder under the agreed Phase 1 structure is settled through payroll with the final pay run and does not form part of this invoice.

## Payment details

| | |
|---|---|
| Account name: | [insert] |
| BSB: | [insert] |
| Account number: | [insert] |
| Reference: | [invoice number] |

Enquiries: agirton@intradark.com
