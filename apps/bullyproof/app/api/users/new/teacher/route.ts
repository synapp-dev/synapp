...
export async function POST(request: Request) {
  try {
    ...
    if (listUsersError) {
      console.error("[TEACHER CREATE] Database error - Failed to list users:", {
    ...
    if (existingUser) {
      ...
      if (createError) {
        console.error(
          "[TEACHER CREATE] Database error - Failed to create user:",
          {
            error: createError,
      ...
      if (!newUser.user) {
        console.error("[TEACHER CREATE] Database error - No user returned:", {
          newUser,
          email: data.email,
        });
