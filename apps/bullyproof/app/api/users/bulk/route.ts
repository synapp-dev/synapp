...
export async function POST(request: Request) {
  ...
  try {
    ...
    // Helper function to create or update user profile using Drizzle
    const ensureUserProfile = async (
      ...
      // Wait a bit for trigger to create profile if it's a new user
      if (!existingProfile) {
        await new Promise((resolve) => setTimeout(resolve, 500));
      }

      // Use provided profile or check if profile exists using Drizzle, retry a few times if needed
      let profile: typeof userProfile.$inferSelect | null =
        existingProfile || null;
      let profileExists = !!existingProfile;
      const maxProfileCheckAttempts = existingProfile ? 0 : 5;

      if (!profileExists) {
        for (let attempt = 1; attempt <= maxProfileCheckAttempts; attempt++) {
          try {
            ...
            console.error(
              `[BULK USER CREATE] Error checking profile (attempt ${attempt}):`,
              error.message
            );
          }

          if (attempt < maxProfileCheckAttempts) {
            await new Promise((resolve) => setTimeout(resolve, 200 * attempt));
          }
        }
      }

      // Build update data using Drizzle field names (camelCase)
      const updateData: {
      ...
      // Set firstName/lastName if provided
      ...
      const currentMetadata =
        (profile?.metadata as UserMetadata | null) || ({} as UserMetadata);
      ...
      if (!profileExists) {
        // Profile doesn't exist - create it using Drizzle
        const maxAttempts = 5;
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
          try {
            ...
            console.log(
              `[BULK USER CREATE] Created profile for ${email} using Drizzle`
            );
            return;
          } catch (error: any) {
            if (error.code === "23505") {
              ...
              console.log(
                `[BULK USER CREATE] Profile for ${email} created by trigger, updating with Drizzle`
              );
              // Update the trigger-created profile
              ...
              if (firstName !== undefined)
                triggerUpdateData.firstName =
                  firstName && firstName.trim() ? firstName.trim() : null;
              if (lastName !== undefined)
                triggerUpdateData.lastName =
                  lastName && lastName.trim() ? lastName.trim() : null;
              triggerUpdateData.metadata = {
                updateLogs: [
                  {
                    type: "creation",
                    updatedAt: new Date().toISOString(),
              ...
              await db
                .update(userProfile)
                .set(triggerUpdateData)
                .where(eq(userProfile.id, targetUserId));

              console.log(
                `[BULK USER CREATE] Updated trigger-created profile for ${email}`
              );
              return;
            }
        ...
        // Profile exists - update it using Drizzle
        ...
        if (hasNameUpdates || needsMetadataUpdate) {
          console.log(
            `[BULK USER CREATE] Updating profile for ${email} using Drizzle:`,
            {
          ...
          try {
            ...
            if (updatedProfile) {
              console.log(
                `[BULK USER CREATE] Successfully updated profile for ${email}:`,
                {
                  savedFirstName: updatedProfile.firstName,
                  savedLastName: updatedProfile.lastName,
                }
              );
            }
          } catch (error: any) {
            console.error(
              `[BULK USER CREATE] Failed to update profile for ${email}:`,
              error.message
            );
          ...
          console.log(
            `[BULK USER CREATE] Profile for ${email} already up to date`
          );
        }
      }
    };
