-- Create a view that combines course topics with enriched data:
-- - Slide counts
-- - Quiz existence (whether topic has quizzes with questions)
-- - User quiz completion status (user-scoped via auth.uid())
-- - Latest quiz score percentage
...
SELECT 
  ...
  -- Has quiz flag (whether topic has any quizzes with questions, regardless of status)
