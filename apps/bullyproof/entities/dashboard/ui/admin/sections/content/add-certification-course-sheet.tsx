...
interface AddCertificationCourseSheetProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  onCourseCreated?: () => void;
}

export function AddCertificationCourseSheet({
  open,
  onOpenChange,
  onCourseCreated,
}: AddCertificationCourseSheetProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const result = await certificationApi.courses.create({
        code: code.trim(),
        name: name.trim(),
      ...
      if (result.error) {
        setError(result.error.message || "Failed to create course");
        return;
      }
      ...
      console.error("Failed to create certification course:", err);
      setError(
        err instanceof Error
          ? err.message
          : "Failed to create certification course. Please try again."
      );
    } finally {
        ...
        <div className="p-6 pb-4 border-b">
          <SheetHeader className="space-y-1">
            ...
            <SheetDescription className="text-sm">
              Create a new certification course by filling in the details below.
            </SheetDescription>
          </SheetHeader>
        </div>
