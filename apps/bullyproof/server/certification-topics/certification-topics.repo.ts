...
export const certificationTopicsRepo = {
  getByStageId: (courseId: string) =>
    db
      .select()
      .from(courseTopics)
      .where(eq(courseTopics.courseId, courseId))
      .orderBy(
        asc(courseTopics.courseOrder),
        asc(courseTopics.title)
      ),

  getByStageCode: async (courseCode: string) => {
  ...
  create: async (data: {
    ...
    return db.insert(courseTopics).values({
  ...
  ) => db.update(courseTopics).set(data).where(eq(courseTopics.id, id)).returning(),

  delete: (id: string) => db.delete(courseTopics).where(eq(courseTopics.id, id)),
