-- Ensure admin audit/support feature keys exist and grant INTRADARK_DEV access.
...
WHERE fp.feature_id = f.id
  ...
  AND f.key IN ('/admin/audit-logs', '/admin/support-tools')
...
FROM features f
CROSS JOIN roles r
WHERE f.key IN ('/admin/audit-logs', '/admin/support-tools')
  AND r.key = 'INTRADARK_DEV'
  AND NOT EXISTS (
    SELECT 1
